...
FastAPI / Cloud Run
  ...
  ├─ QWeather -> Open-Meteo -> last-good cache -> neutral
...
- QWeather 连接/总超时约 1.2/2.0 秒；Open-Meteo 连接/总超时约 1.0/1.8 秒；服务层总预算约 3.5 秒。
- 成功即写 last-good；主源失败才调备用；双失败读 stale；无 stale 返回 neutral。
- 结构化日志只写 provider、阶段、异常类、耗时、粗粒度网格与 request id，不写 key 和完整坐标。

WeatherData 新增字段有默认值，旧快照仍可反序列化：`source=qweather|open_meteo|cache|neutral`、`is_stale`、`observed_at?`。前端只缓存 providerAvailable 的真实结果。
...
- 天气：清空 QWeather 配置后自动回到 Open-Meteo+neutral。

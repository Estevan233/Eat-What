...
function storageStub(initial = '') {
  let cache = initial
  return {
    getStorageSync: vi.fn((key: string) => key === 'eat_what_meal_recommendation_v1' ? cache : ''),
    setStorageSync: vi.fn((key: string, value: string) => {
      if (key === 'eat_what_meal_recommendation_v1') cache = value
    }),
...
describe('daily complete meal store', () => {
  ...
  it('saves a versioned cache and restores it as stale read-only state', async () => {
    ...
    const write = vi.mocked(uni.setStorageSync).mock.calls.find(
      ([key]) => key === 'eat_what_meal_recommendation_v1',
    )
    expect(JSON.parse(String(write?.[1])).version).toBe(1)

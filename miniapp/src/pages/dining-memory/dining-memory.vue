<template>
  <view class="page">
    <view class="intro">
      ...
      <text class="intro-copy">只对“店铺＋菜品”这组组合生效。喜欢会优先想起，避雷会被排除；不会因为一家店的一道菜翻车，就把整家店判死刑。</text>
    </view>
    ...
    <view class="list-head">
      <view>
        ...
        <text class="list-count">{{ diningStore.memories.length }} 条</text>
      </view>
    ...
    <view v-else class="empty">
      ...
      <text class="empty-copy">下次吃到惊喜或踩雷，别只在脑子里骂两句。</text>
    </view>
  </view>
</template>
...
.page { min-height: 100vh; padding: 32rpx 32rpx 80rpx; box-sizing: border-box; background: $bg; }
.intro { display: flex; flex-direction: column; gap: 10rpx; padding: 20rpx 8rpx 30rpx; }
.intro-kicker { color: $brand; font-size: 18rpx; font-weight: 800; letter-spacing: 3rpx; }
.intro-title { color: $ink; font-size: 40rpx; font-weight: 850; }
.intro-copy { color: $ink-2; font-size: 22rpx; line-height: 1.65; }
.form-card { padding: 30rpx; border-radius: $radius-lg; background: $card; box-shadow: $shadow-card; }
.field { position: relative; display: flex; flex-direction: column; gap: 12rpx; margin-bottom: 24rpx; }
.label { color: $ink; font-size: 23rpx; font-weight: 700; }
.input, .textarea { padding: 0 22rpx; border: 1rpx solid $line; border-radius: 20rpx; color: $ink; background: $bg; font-size: 23rpx; box-sizing: border-box; }
.input { height: 80rpx; }
.textarea { width: 100%; height: 180rpx; padding-top: 20rpx; padding-bottom: 36rpx; line-height: 1.55; }
.counter { position: absolute; right: 16rpx; bottom: 12rpx; color: $ink-3; font-size: 17rpx; }
.verdicts { display: flex; gap: 12rpx; }
.verdict { flex: 1; padding: 15rpx 10rpx; border: 1rpx solid $line; border-radius: 18rpx; color: $ink-2; background: $bg; font-size: 21rpx; text-align: center; }
...
.save { height: 90rpx; line-height: 90rpx; margin: 6rpx 0 0; border-radius: 999rpx; color: #fff; background: $grad-brand; box-shadow: $shadow-cta; font-size: 27rpx; font-weight: 750; }
.save::after { border: none; }
.save[disabled] { color: #fff; opacity: .48; }
.list-head { display: flex; justify-content: space-between; align-items: flex-end; gap: 20rpx; margin: 38rpx 6rpx 18rpx; }
